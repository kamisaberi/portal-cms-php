...
if (isset($_GET['id']) && is_numeric($_GET['id'])) {
    $post = ExamController::FindOneExam($_GET['id']);
    $cm = 'edit';
}
?>
...
<main class="container-fluid fixed-nav-offset">
    <div class="row">
        <form action="OperateExam.php" method="POST" class="form-horizontal">
            <input type="hidden" id="cm" name="cm" value="<?php echo $cm ?>" />
            <?php echo $cm == 'edit' ? "<input type='hidden' id='ExamId' name='ExamId' value='$post->ExamId' />" : ''; ?>
                ...
                <div class="panel panel-fav">
                    ...
                    <div class="panel-body">
                        <div class="col-sm-12">
                            ...
                            <div class="form-group">
                                <label class="control-label col-sm-3" for="ExamType">
                                    نوع آزمون:
                                </label>
                                <div class="col-sm-9">
                                    <select name="ExamType" id="ExamType" class="form-control" >
                                        <?php
                                        $expertparents = ExamTypeController::Fill();
                                        foreach ($expertparents as $expertparent) {
                                            $selected = "";
                                            if ($cm == 'edit') {
                                                if ($post->ExamType->ExamTypeId == $expertparent->ExamTypeId) {
                                                    $selected = "selected";
                                                }
                                            }
                                            echo "<option value='" . $expertparent->ExamTypeId . "' " . $selected . ">" . $expertparent->ExamTypeId . " : " . $expertparent->Title . "</option>";
                                        }
                                        ?>
                                    </select>
                                </div>
